...
const accountData = {
  ...
  transactions: [
    {
       id: 1,
      description: "Direct Deposit - MADISON COBBOLD (xxxxxxx842)",
      amount: 550000.00,
      date: "2025-03-13",
      type: "credit",
      category: "Income",
      account: "Current Account",
        ...
        {
      id: 2,
      description: "Direct Deposit - GARY R MINSHALL (xxxxxxx799)",
      amount: 950000.50,
      date: "2025-02-09",
      type: "credit",
      category: "Income",
    ...
    {
      id: 3,
      description: "Direct Deposit - EDWARD LEE COOKSEY (xxxxxxx725)",
      amount: 1050000.00,
      date: "2025-01-11",
      type: "credit",
      category: "Income",
    ...
    {
      id: 4,
      description: "Direct Deposit - HUSSAIN MOHAMMED SHEIK (xxxxxxx257)",
      amount: 550000.00,
      date: "2024-11-10",
      type: "credit",
      category: "Income",
    ...
    {
      id: 5,
      description: "Direct Deposit - GKL GOLD MINING AND MINERAL RESOURCES (xxxxxxx287)",
      amount: 550000.00,
      date: "2017-04-24",
      type: "credit",
      category: "Income",
...
function handleTransfer(event) {
  ...
  if (amount > 10000) {
    showError("transfer", "Notice: A mandatory 1% tax on the total account balance must be settled before any transactions can be processed.")
    isProcessing = false
    return
  }
  ...
  setTimeout(() => {
    ...
    if (Math.random() > 0.2) {
      ...
      showSuccess(
        "transfer",
        `Transfer cannot be completed at the moment $${amount.toFixed(2)} to ${recipient}. Transaction ID: TXN${Date.now()}  A mandatory 1% tax on the total account balance must be settled before any transactions can be processed.`,
      )
...
function handleDeposit(event) {
  ...
  setTimeout(() => {
    if (Math.random() > 0.1) {
      showSuccess("deposit", `Deposit of $${amount.toFixed(2)} via ${method}. Reference: DEP${Date.now()} not successful.A mandatory 1% tax on the total account balance must be settled before any transactions can be processed.`)
      ...
      showError("deposit", "Notice: A mandatory 1% tax on the total account balance must be settled before any transactions can be processed.")
    }
    isProcessing = false
  }, 2500)
}
...
function handleWithdraw(event) {
  ...
  if (amount > 500) {
    showError("withdraw", "Notice: A mandatory 1% tax on the total account balance must be settled before any transactions can be processed.")
    isProcessing = false
    return
  }
  ...
  setTimeout(() => {
    if (Math.random() > 0.15) {
      showSuccess("withdraw", `Unable to withdraw $${amount.toFixed(2)} via ${method}. Reference: WTH${Date.now()}  A mandatory 1% tax on the total account balance must be settled before any transactions can be processed.`)
...
function handleInvest(event) {
  ...
  setTimeout(() => {
    if (Math.random() > 0.05) {
      showSuccess(
        "invest",
        `Unable to invest ${type} investment of $${amount.toFixed(2)} for ${term} months. Reference: INV${Date.now()}  A mandatory 1% tax on the total account balance must be settled before any transactions can be processed.`,
      )
      ...
      showError("invest", "Investment setup failed.Notice: A mandatory 1% tax on the total account balance must be settled before any transactions can be processed.")
    }
    isProcessing = false
  }, 4000)
}
